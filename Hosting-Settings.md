...
3. We now have 2 TXT records (step 2 on the SSL for free website). We will copy back the TXT records to the domain provider. Leave the SSL for free page open. We will come back to it.
4. On the domain provider website, select your domain > Manage DNS. Let's create a new TX record. Repeat the steps for both TXT records on SSL for free.

   **Name**: _acme-challenge.{yourdomain.com} (copied from SSL for free)

   **Type**: TXT.

   **TTL**: 300 (if you can set it to 1, go for it).

   **Target**: qariKbGVafpb5MespqHdMaGi933yW-Z2A3shE0wIyZA (copied from SSL for free, yours will look similar).
...
#### Setup Emby with your SSL certificate
...
3. Go to your Emby server dashboard > Advanced. Save your changes and restart your server.
   ...
   **Custom SSL certificate path**: point to your newly created certificate.pem
